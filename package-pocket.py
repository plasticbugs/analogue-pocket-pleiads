...
RBF = os.path.join(ROOT, "projects", "output_files", "pleiads_pocket.rbf")
PKG = os.path.join(ROOT, "pkg", "pocket")
OUT = os.path.join(ROOT, "release", "pocket")
CORE_ID = "plasticbugs.pleiads"
...
# release contains everything needed to produce pleiads.rom.
for extra in ("pleiads.mra", "phoenix.mra", "README.md",
...
# Backstop: the instance JSONs are how the Pocket lists the games, and how an
# updater learns which ROM files to fetch. Losing one silently drops a game from
# the menu, which looks like a core bug rather than a packaging one.
inst = os.path.join(OUT, "Assets", PLATFORM_ID, CORE_ID)
want = {"Pleiads.json", "Phoenix.json"}
...
print("build the ROM with:  python3 mra_build.py pleiads.mra pleiads.zip")
print("                 or:  python3 mra_build.py phoenix.mra phoenix.zip")
